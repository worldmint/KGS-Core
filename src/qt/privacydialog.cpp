...
#include "zkingscoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zKGS ought to be enough for anybody." - Bill Gates, 2017
    ui->zKGSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzKGSSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzKGS->setEnabled(false);
        ui->pushButtonMintzKGS->setToolTip(tr("zKGS is currently disabled due to maintenance."));

        ui->pushButtonSpendzKGS->setEnabled(false);
        ui->pushButtonSpendzKGS->setToolTip(tr("zKGS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zKGSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzKGS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zKGS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zKGS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zKGS in ") +
...
void PrivacyDialog::on_pushButtonSpendzKGS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zKGS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zKGS
        sendzKGS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zKGS
    sendzKGS();
}

void PrivacyDialog::on_pushButtonZKingscoinControl_clicked()
{
    ZKingscoinControlDialog* zKingscoinControl = new ZKingscoinControlDialog(this);
    zKingscoinControl->setModel(walletModel);
    zKingscoinControl->exec();
}

void PrivacyDialog::setZKingscoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzKingscoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzKGS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Kingscoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zKGSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zKGS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " KGS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zKGS</b>";
    ...
    // use mints from zKingscoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZKingscoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZKingscoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zkingscoin selector in case it was used
    ZKingscoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zKingscoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Kingscoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zKingscoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zKGS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zKGS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zKGS "));
    ui->labelzKGSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzKGSSyncStatus->setVisible(fShow);
}
