...
#include "ui_zkingscoincontroldialog.h"
...
std::list<std::string> ZKingscoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZKingscoinControlDialog::listMints;

ZKingscoinControlDialog::ZKingscoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZKingscoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZKingscoinControlDialog::~ZKingscoinControlDialog()
{
    delete ui;
}

void ZKingscoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZKingscoinControlDialog::updateList()
{
...
void ZKingscoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZKingscoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZKingscoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZKingscoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZKingscoinControlDialog::GetSelectedMints()
{
...
void ZKingscoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
