...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the KGS and zKGS money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 5545, 6666));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, kingscoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Kingscoin specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:5545"));
    ...
    strUsage += HelpMessageOpt("-backupzkingscoin=<n>", strprintf(_("Enable automatic wallet backups triggered after each zKingscoin minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizekingscoinamount=<n>     " + strprintf(_("Keep N KGS anonymized (default: %u)"), 0) + "\n";
            ...
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZKGSMinted();
                        RecalculateZKGSSpent();
                    }
                    RecalculateKGSSupply(1);
                }
                ...
                // Kingscoin: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZKingscoinBackups = GetBoolArg("-backupzkingscoin", true);
        pwalletMain->setZKingscoinAutoBackups(fEnableZKingscoinBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeKingscoinAmount = GetArg("-anonymizekingscoinamount", 0);
//    if (nAnonymizeKingscoinAmount > 999999) nAnonymizeKingscoinAmount = 999999;
//    if (nAnonymizeKingscoinAmount < 2) nAnonymizeKingscoinAmount = 2;
    ...
    LogPrintf("Anonymize Kingscoin Amount %d\n", nAnonymizeKingscoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1KGS+1000 == (.1KGS+100)*10
       10KGS+10000 == (1KGS+1000)*10
    */
